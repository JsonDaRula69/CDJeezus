...
CDJeez monitors SoundCloud playlists for new tracks, searches Soulseek for
...
├── config.py            # Env-based config via .env in ~/.config/cdjeez/
...
SoundCloud API → metadata (artist, title, duration)
  ...
  → Download to staging (~/.config/cdjeez/staging/)
...
- **DO NOT** assume plist name is stable — handle both `com.djtchill.cdjeez` (legacy) and `com.cdjeez` (current)
...
### Updater Rules
- `cdjeez update`: fetches latest version, stops daemon, upgrades, migrates data, restarts
...
- **Plist rename**: v0.12.1 changed plist from `com.djtchill.cdjeez` to `com.cdjeez`. Uninstall must check BOTH names. Setup must unload old plist if it exists.
- **Also check**: `com.djtchill.cdjeez` (legacy name from v0.12.1)
...
- **Staging directory**: `~/.config/cdjeez/staging/` — NOT inside `_Serato_` folder (minimizes external activity in sensitive Serato directory).
- **Graceful shutdown**: `cdjeez stop` writes a flag file and sends SIGUSR1. The daemon checks `should_stop()` between operations and uses `asyncio.wait_for(_stop_event.wait(), timeout=poll_interval)` so SIGUSR1 wakes it from sleep immediately.
- **Single instance**: Running `cdjeez` when a daemon is already running tails the log file instead of starting a duplicate. `--force` overrides this.
- **Log file**: `~/.config/cdjeez/cdjeez.log` (rotating, 5MB max, 3 backups).
- **PID file**: `~/.config/cdjeez/cdjeez.pid`
